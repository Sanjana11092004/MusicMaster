...
// Sample video/course data - same as before
const videoData = [
...
const instruments = ["All", "Guitar", "Violin", "Singing", "Flute", "Piano"];

const BrowseCourses = () => {
  const [selectedInstrument, setSelectedInstrument] = useState("All");

  // Filter courses by selected instrument or show all
  const filteredCourses =
    selectedInstrument === "All"
      ? videoData
      : videoData.filter((c) => c.instrument === selectedInstrument);

  return (
    <>
      <Header />
      <div className="browsecourses-container">
        <nav className="browse-nav">
          {instruments.map((inst) => (
            <button
              key={inst}
              className={`browse-nav-btn ${
                selectedInstrument === inst ? "active" : ""
              }`}
        ...
        <div className="browse-grid">
          {filteredCourses.length === 0 ? (
            <p className="browse-empty">No courses available</p>
          ) : (
            filteredCourses.map((course) => (
              <div className="cv-card" key={course.id}>
                <img
                  src={course.img}
                  alt={course.title}
                  className="cv-img"
                />
                <h3 className="cv-name">{course.title}</h3>
                <p className="cv-desc">{course.description}</p>
                <div className="cv-meta">
                  <span className="cv-level">{course.level}</span>
                  <span className="cv-duration">⏱ {course.duration}</span>
                </div>
              </div>
            ))
          )}
